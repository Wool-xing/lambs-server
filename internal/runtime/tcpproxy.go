...
// idleOnce is one idle-monitor pass — extracted so the stop logic is
// testable without the never-returning monitor loop (same shape as the
// ProcManager.healthOnce extraction).
func (tp *TCPProxy) idleOnce() {
	tp.mu.RLock()
	defer tp.mu.RUnlock()
